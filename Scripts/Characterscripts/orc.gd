...
func _update_sight() -> void: #funktion som mha raycast jagar spelarn om den är synlig
	"
	Funktion som mha raycast jagar spelarn om den är synlig. Raycasten riktas från orcen till spelaren
	Om den kolliderar med något annat än spelaren, och spelaren därmed är ur orcens syn, såens tappar orcen spelaren ur sikte
...
func _take_damage():
	"
	Anropas när spelaren skadar orcen. Om hp = 0 anropas _enter_dead_state
...
func _walk_state(delta: float) -> void:
	"
	Orcen går mot spelaren om raycasten kolliderar med spelaren. Om slow chase är true blir han långsam. 
...
func _attack_state(delta:float) -> void:
	"
	Attack animation körs i left, right, up eller down. I orcen (till skillnad från enemyn) sköts animationerna i animationplayer
	så attacken är mer precis och fungerar bättre (jag gjorde orcsen senare än enemys, så insåg att animationplayer är 100x bättre för attacker).
	Om spelaren är 70 pixlar bort anropas _enter_walk_state.
	"
	_movement(delta, Vector2.ZERO, 0)
	
	if $AttackTimer.is_stopped(): #Attack m cooldown
		animation_player.play("Attack_" + direction_name)
		$AttackTimer.start()
		
	var distance_to_player = global_position.distance_to(player.global_position)
	if distance_to_player > 70:
		await animation_player.animation_finished #så han inte avbryter slaget
		_enter_walk_state()
...
func _on_area_2d_body_entered(body: Node2D) -> void:
	"
	Orcen har en detection area, så den börjar enbart jaga när spelaren först går in i detection arean.
	"
...
func _on_damage_player_area_body_entered(body: Node2D) -> void:
	"
	Attack_arean som sköts i AnimationPlayern, när orcen attackerar.
	"
	if body.name == "Player":
